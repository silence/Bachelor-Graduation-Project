...
PATH = '/Users/apple/Downloads/ml-dl-nlp-大数据等比赛/kesci大数据挑战赛/dataset/'
...
def get_train_label1():
    ...
    train_data_id = np.unique(
        pd.concat([train1_create['user_id'], train1_launch['user_id'], train1_activity['user_id']]))
...
def get_train_label2():
    ...
    train_data_id = np.unique(
        pd.concat([train2_create['user_id'], train2_launch['user_id'], train2_activity['user_id']]))
...
def feature_extraction(data_id, begin_day, end_day):
    ...
    # 拍摄视频的天数
    video_days = video.drop_duplicates(['user_id', 'create_day']).groupby(['user_id']).size().rename(
        'create_days').reset_index()
    ...
    # 一天中创建视频的最多，最少数量
    create_num_max_one_day = create_num_day.groupby(['user_id'])['create_num_day'].max().rename(
        'create_num_max_one_day').reset_index()
    data_id = pd.merge(data_id, create_num_max_one_day, on='user_id', how='left')
    create_num_min_one_day = create_num_day.groupby(['user_id'])['create_num_day'].min().rename(
        'create_num_min_one_day').reset_index()
    ...
    video7_days = video7.drop_duplicates(['user_id', 'create_day']).groupby(['user_id']).size().rename(
        'create7_days').reset_index()
    ...
    # 登录天数
    launch_days = launch.drop_duplicates(['user_id', 'launch_day']).groupby(['user_id']).size().rename(
        'launch_days').reset_index()
    ...
    # 一天中登录的最多，最少次数
    launch_num_max_one_day = launch_num_day.groupby(['user_id'])['launch_num_day'].max().rename(
        'launch_num_max_one_day').reset_index()
    data_id = pd.merge(data_id, launch_num_max_one_day, on='user_id', how='left')
    launch_num_min_one_day = launch_num_day.groupby(['user_id'])['launch_num_day'].min().rename(
        'launch_num_min_one_day').reset_index()
    ...
    # 有activity的总天数
    activity_days = activity.drop_duplicates(['user_id', 'activity_day']).groupby(['user_id'])[
        'activity_day'].count().rename('activity_days').reset_index()
    ...
    activity_day_median = activity.groupby(['user_id'])['activity_day'].median().rename(
        'activity_day_median').reset_index()
    ...
    activity_day_mode = activity.groupby(['user_id'])['activity_day'].apply(mode).rename(
        'activity_day_mode').reset_index()
    ...
    # 每个用户，每天的行为次数
    activity_day_nums = activity.groupby(['user_id', 'activity_day'])['page'].count().rename(
        'activity_day_nums').reset_index()
    # 求每天行为次数的mean,std,等
    activity_day_nums_max = activity_day_nums.groupby(['user_id'])['activity_day_nums'].max().rename(
        'activity_day_nums_max').reset_index()
    data_id = pd.merge(data_id, activity_day_nums_max, on='user_id', how='left')
    activity_day_nums_min = activity_day_nums.groupby(['user_id'])['activity_day_nums'].min().rename(
        'activity_day_nums_min').reset_index()
    data_id = pd.merge(data_id, activity_day_nums_min, on='user_id', how='left')
    activity_day_nums_mean = activity_day_nums.groupby(['user_id'])['activity_day_nums'].mean().rename(
        'activity_day_nums_mean').reset_index()
    data_id = pd.merge(data_id, activity_day_nums_mean, on='user_id', how='left')
    activity_day_nums_var = activity_day_nums.groupby(['user_id'])['activity_day_nums'].var().rename(
        'activity_day_nums_var').reset_index()
    ...
    # 每个用户每个video的观看次数
    activity_video_nums = activity.groupby(['user_id', 'video_id'])['page'].count().rename(
        'activity_video_nums').reset_index()
    # 观看同一video的最大最小次数
    activity_video_nums_max = activity_video_nums.groupby(['user_id'])['activity_video_nums'].max().rename(
        'activity_video_nums_max').reset_index()
    data_id = pd.merge(data_id, activity_video_nums_max, on='user_id', how='left')
    activity_video_nums_min = activity_video_nums.groupby(['user_id'])['activity_video_nums'].min().rename(
        'activity_video_nums_min').reset_index()
    data_id = pd.merge(data_id, activity_video_nums_min, on='user_id', how='left')

    # 每个用户每个author的观看次数
    activity_author_nums = activity.groupby(['user_id', 'author_id'])['page'].count().rename(
        'activity_author_nums').reset_index()
    # 观看同一author的最大最小次数
    activity_author_nums_max = activity_author_nums.groupby(['user_id'])['activity_author_nums'].max().rename(
        'activity_video_nums_max').reset_index()
    data_id = pd.merge(data_id, activity_author_nums_max, on='user_id', how='left')
    activity_author_nums_min = activity_author_nums.groupby(['user_id'])['activity_author_nums'].min().rename(
        'activity_video_nums_min').reset_index()
    ...
    page0_day_max = activity[activity['page'] == 0].groupby(['user_id'])['activity_day'].max().rename(
        'page0_day_max').reset_index()
    data_id = pd.merge(data_id, page0_day_max, on='user_id', how='left')
    page1_day_max = activity[activity['page'] == 1].groupby(['user_id'])['activity_day'].max().rename(
        'page1_day_max').reset_index()
    data_id = pd.merge(data_id, page1_day_max, on='user_id', how='left')
    page2_day_max = activity[activity['page'] == 2].groupby(['user_id'])['activity_day'].max().rename(
        'page2_day_max').reset_index()
    data_id = pd.merge(data_id, page2_day_max, on='user_id', how='left')
    page3_day_max = activity[activity['page'] == 3].groupby(['user_id'])['activity_day'].max().rename(
        'page3_day_max').reset_index()
    data_id = pd.merge(data_id, page3_day_max, on='user_id', how='left')
    page4_day_max = activity[activity['page'] == 4].groupby(['user_id'])['activity_day'].max().rename(
        'page4_day_max').reset_index()
    ...
    action_type0_num = activity[activity['action_type'] == 0].groupby(['user_id'])['action_type'].count().rename(
        'action_type0_num').reset_index()
    data_id = pd.merge(data_id, action_type0_num, on='user_id', how='left')
    action_type1_num = activity[activity['action_type'] == 1].groupby(['user_id'])['action_type'].count().rename(
        'action_type1_num').reset_index()
    data_id = pd.merge(data_id, action_type1_num, on='user_id', how='left')
    action_type2_num = activity[activity['action_type'] == 2].groupby(['user_id'])['action_type'].count().rename(
        'action_type2_num').reset_index()
    data_id = pd.merge(data_id, action_type2_num, on='user_id', how='left')
    action_type3_num = activity[activity['action_type'] == 3].groupby(['user_id'])['action_type'].count().rename(
        'action_type3_num').reset_index()
    data_id = pd.merge(data_id, action_type3_num, on='user_id', how='left')
    action_type4_num = activity[activity['action_type'] == 4].groupby(['user_id'])['action_type'].count().rename(
        'action_type4_num').reset_index()
    data_id = pd.merge(data_id, action_type4_num, on='user_id', how='left')
    action_type5_num = activity[activity['action_type'] == 5].groupby(['user_id'])['action_type'].count().rename(
        'action_type5_num').reset_index()
    ...
    action_type0_day_max = activity[activity['action_type'] == 0].groupby(['user_id'])['activity_day'].max().rename(
        'action_type0_day_max').reset_index()
    data_id = pd.merge(data_id, action_type0_day_max, on='user_id', how='left')
    action_type1_day_max = activity[activity['action_type'] == 1].groupby(['user_id'])['activity_day'].max().rename(
        'action_type1_day_max').reset_index()
    data_id = pd.merge(data_id, action_type1_day_max, on='user_id', how='left')
    action_type2_day_max = activity[activity['action_type'] == 2].groupby(['user_id'])['activity_day'].max().rename(
        'action_type2_day_max').reset_index()
    data_id = pd.merge(data_id, action_type2_day_max, on='user_id', how='left')
    action_type3_day_max = activity[activity['action_type'] == 3].groupby(['user_id'])['activity_day'].max().rename(
        'action_type3_day_max').reset_index()
    data_id = pd.merge(data_id, action_type3_day_max, on='user_id', how='left')
    action_type4_day_max = activity[activity['action_type'] == 4].groupby(['user_id'])['activity_day'].max().rename(
        'action_type4_day_max').reset_index()
    data_id = pd.merge(data_id, action_type4_day_max, on='user_id', how='left')
    action_type5_day_max = activity[activity['action_type'] == 5].groupby(['user_id'])['activity_day'].max().rename(
        'action_type5_day_max').reset_index()
...
def predcit_cv():
    ...
    for i in range(5):
        ...
        lgb_clf = lgb.LGBMClassifier(boosting_type='gbdt', num_leaves=63, max_depth=-1,
                                     ...
                                     colsample_bytree=0.7, reg_alpha=0.0, reg_lambda=0.1, random_state=100 * i + 500,
                                     n_jobs=-1,
                                     )
